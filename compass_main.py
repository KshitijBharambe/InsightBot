    ...
    def initialize_databases(self):
        ...
        try:
            ...
            living_expenses_df = pd.read_csv(
                os.path.join(self.data_path, "Avg_Living_Expenses.csv")
            )
            employment_df = pd.read_csv(
                os.path.join(self.data_path, "Employment_Projections.csv")
            )
            university_text = self.load_word_document(
                os.path.join(self.data_path, "University_Data.docx")
            )

            # Process living expenses
            living_expenses_loader = DataFrameLoader(
                living_expenses_df, page_content_column="State"
            )
            living_expenses_docs = living_expenses_loader.load()

            # Process employment projections
            employment_loader = DataFrameLoader(
                employment_df, page_content_column="Occupation Title"
            )
    ...
    def get_recommendations(self, query: str) -> str:
        ...
        try:
            ...
            if st.session_state.user_preferences:
                ...
                enhanced_query = (
                    f"Briefly answer: {query} Keep response concise and focused."
                )
